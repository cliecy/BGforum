...
class UserCURD:
    ...
    @classmethod
    def getUserByUserId(cls, userId, newSession):
    ...
    @classmethod
    def getUserByUserName(cls, userName, newSession):
        with newSession() as s:
    ...
    @classmethod
    async def updateUser(cls, receivedJson, newSession):
        jsondict = json.loads(receivedJson)
        date_format = "%Y-%m-%d %H:%M:%S"
    ...
    @classmethod
    async def deleteUserByUserId(cls, receivedJson, newSession, userId):
        jsondict = json.loads(receivedJson)
        date_format = "%Y-%m-%d %H:%M:%S"
        try:
            # 查找现有用户
            async with newSession() as s:
                stmt = select(User).where(User.UserId == userId)
                result = await s.execute(stmt)
                user = result.scalars().first()
